...
object SettingUtil {
    ...
    fun getAppCfg(packageName: String?, className: String?): AppCfg {
        ...
        return if(cfg.getString("scroll") == "ACTION") AppCfg("ACTION", Float.NaN, Float.NaN, Float.NaN, 0)
                else AppCfg(cfg.getString("scroll"), cfg.optDouble("y1").toFloat(), cfg.optDouble("y2").toFloat(), cfg.optDouble("mark").toFloat(), cfg.optLong("duration"))
    }
    ...
    data class AppCfg(val scroll: String, val y1: Float, val y2: Float, val mark: Float, val duration: Long)
}
